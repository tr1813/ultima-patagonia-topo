...
output_file_name = '{name}-{projection_short}.{format}'.format(
    name=str(survey.name).strip('"'), projection_short=PROJECTION[0], format=FORMAT)
output_path = join(dirname(survey.file_path), output_file_name)
...
endscrap"""
    ...
    if not isfile(output_path):
        print("THERION OUTPUT PATH", output_path)
