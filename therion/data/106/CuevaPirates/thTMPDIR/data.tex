...
% LuaTex compatibility definitions taken from the LuaTeX manual:
\ifx\directlua\undefined\else
    ...
    \def\pdffontname      {\pdffeedback fontname}    % modified
    \def\pdffontobjnum    {\pdffeedback fontobjnum}  % modified
    ...
    \edef\pdfsuppressptexinfo         {\pdfvariable suppressoptionalinfo }
    ...
    % the next three can't be defined inside \ifx..\fi;
    % if required, they could be loaded from external file
...
\let\colorlegendsymbolbox=\legendsymbolbox
...
\def\setref#1#2{\expandafter\edef\csname thREFS:#1\endcsname{#2}}
\def\getref#1{%
  \ifcsname thREFS:#1\endcsname
    \csname thREFS:#1\endcsname
  \else
    \csname #1\endcsname   % fallback for northarrow/scalebar when using registers
  \fi
}
...
\def\modelcmyk{cmyk}
\def\modelrgb{rgb}
\def\modelgray{grayscale}
\def\black{\pdfliteral{\ifx\OutputColormodel\modelcmyk 0 0 0 1 k\else
           \ifx\OutputColormodel\modelrgb 0 0 0 rg\else
           0 g\fi\fi}}
  ...
  \line{%
    \vbox to \ht\navbox{
      ...
      \ifpagenumbering
        ...
        \hbox{\qquad\qquad
          \vtop{%
            \hbox to 0pt{\hss\strut\showpointer\pointerN\hss}
            \hbox to 0pt{\llap{\showpointer\pointerW\hskip0.8em}%
              \raise3pt\hbox to 0pt{\hss$\uparrow$\hss}%
              \raise-1pt\hbox to 0pt{\hss$\downarrow$\hss}%
              \raise1pt\hbox to 0pt{\hss$\leftarrow\kern-7pt\rightarrow$\hss}%
              \rlap{\hskip0.8em\showpointer\pointerE}}
              \hbox to 0pt{\hss\strut\showpointer\pointerS\hss}
          }\qquad\qquad
          \vtop{
            \def\arr{$\uparrow$}
...
\newtoks\gridconv
\newtoks\currentdate
\newtoks\legendtextcolor\legendtextcolor={\black}
...
\def\scalebar{\pdfrefxform\getref{THWscalebar}}
\def\altitudebar{\pdfrefxform\getref{THWaltitudebar}}
\def\northarrow{\pdfrefxform\getref{THWnortharrow}}
...
%    \dimtobp{\wd\tmpboxa}\edef\wdth{\tmpdef}%
    ...
    \rlap{\PL{q \the\bgcolor\space \shiftx\space \shifty\space \wdth\space \hght\space re f Q}}%
   \endgroup    
  \fi
  %
...
\newtoks\includeprefix\includeprefix={.}
\newif\ifabspath
\def\checkcolon #1:#2\endparam{\def\tmp{#2}\ifx\tmp\empty\else\abspathtrue\fi}
\def\checkstartslash #1#2\endparam{\if/#1\abspathtrue\fi}

% convert a relative path to absolute and save it under the name #1
\def\savepath#1#2{\expandafter\edef\csname thABS:#1\endcsname{\the\includeprefix #2}}

\def\loadpicture#1{%
  \ifcsname thABS:#1\endcsname   % use a saved absolute path if it exists
    \pdfximage{\csname thABS:#1\endcsname}%
  \else
    \abspathfalse
    \checkcolon #1:\endparam
    \checkstartslash #1\endparam
    \pdfximage{\ifabspath\else\the\includeprefix\fi #1}%
  \fi
  \pdfrefximage\pdflastximage
}

\def\inputrel#1{\input \the\includeprefix #1}
...
\pdfcolorstack 0 set {\ifx\OutputColormodel\modelcmyk 0 0 0 1 k 0 0 0 1 K\else
           \ifx\OutputColormodel\modelrgb 0 0 0 rg 0 0 0 RG\else
           0 g 0 G\fi\fi}
...
\def\maplayout{}
\includeprefix={C:/Users/User/Documents/ultima-patagonia-topo/therion/data/106/CuevaPirates/./}
\def\xcenter{0}
\def\ycenter{90}
\def\xscale{40}
\def\yscale{3}
\def\customheader{\line{something}
\size[13]\bf \the\cavename \ss
\si \par \size[9]Iles Madre de Dios \ss
\par \thfb\char68 \thfc\char233 \thfb\char118 \char101 \char108 \char111 \char112 \char101 \char109 \char101 \char110 \char116 \char58 \mainfont{} \the\cavelength
\par \thfb\char68 \thfc\char233 \thfb\char110 \char105 \char118 \char101 \char108 \thfc\char233 \thfb\char58 \mainfont{} \the\cavedepth
\par \the\topoteam
\par Topographie: Centre Terre, Oxford University Caving Club
\par Ultima Patagonia 2008, 2023}
\def\maplayout{
